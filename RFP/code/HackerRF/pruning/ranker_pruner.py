...
verbose: int
    ...
    earlystopping_callback = tf.keras.callbacks.EarlyStopping(
        monitor="val_loss", patience=10, mode="auto", verbose=verbose
    )
...
def prune_automatic(
    ...
    training_verbose: Annotated[bool, typer.Option()] = True,
    stop_after_data_load: Annotated[bool, typer.Option()] = False,
):
    """
    ...
    X_train, Y_train, X_val, Y_val, X_test, Y_test = load_hacker()

    if verbose or stop_after_data_load:
        tot = X_train.shape[0] + X_val.shape[0] + X_test[0]
        print(f"Total IQ traces {tot}")
        print(f"Train shape: {X_train.shape}")
        print(f"Val shape: {X_val.shape}")
        print(f"Test shape: {X_test.shape}")
    if stop_after_data_load:
        return
    ...
    if not skip_finetune:
        ...
        finetuned_pruned_model, history = finetune_model(
            pruned_model, X_train, Y_train, X_val, Y_val, out, batch_size=batch_size, verbose=training_verbose
        )
        ...
        print(f"Finetune time: {finetune_runtime.total_seconds()}")
        print(f"Finetune epochs: {len(history.history['loss'])}")
        print(f"Finetune sec per epoch: {finetune_runtime.total_seconds()/len(history.history['loss'])}")
...
def class_ranks(model, testX, testY, num_class, preds=None):
    ...
    for i in range(num_class):
        ...
        if preds is None:
            #TODO: NOTICE: Ryan has changed this:
            # OneClsData = np.squeeze(np.take(testX, ids, axis=0))
            # Issue here is this appends a single dimension at the start... just pop it
...
@app.command()
def rank_plot(
    model_path: Annotated[Path, typer.Argument(help='model checkpoint path')],
    ...
    if not use_cache:
        _, _, _, _, X_test, Y_test = load_hack()
        model = load_model(model_path)
        # Load the ranks, return array has <NUM_DEVICE> rows and <NUM_TRACES> 
        # columns. 
        ranks = np.array(class_ranks(model, X_test, Y_test, 10, preds=None))

        #TODO: Should average rank be implementated?

        # Create and df  
        df = pd.DataFrame(ranks, index=None)
        df.to_csv("rank_result/sameday_rank_RYAN.csv",header=False)
    else:
   ...
   #  device3
    ...
    # In the above sequence, want each device to quickly become rank 1 

    # My plot is current rank, vs trace num 

    # To show the mean, show the mean rank for each device
    ...
    fig = plt.plot(x=mean_df[1], y=[i for i in range(0,11)], linewidth=2,label='Train X1_11: Test X2_20' ,marker='^',markevery=20,markersize=10)

...
#    filelist = os.listdir(glob_dat_path)
    ...
    for i,f in enumerate(filelist):
        ...
        if dataType == "IQ":
            ...
            print(f"     has {IQs_per_tran.shape[0]}")
            print(f"     has tot shape: {IQs_per_tran.shape}")
            print(f"     has n samples: {n_samples}")
            print(f"     has running tot smaples: {tot_samples}")
            tot_traces += math.floor(n_samples / 864)
            print(f"     has running trace smaples: {tot_traces}")
        ...
        if not mul_trans:
            ...
            break
    return all_IQ_data, num_tran, tot_samples, tot_traces
...
def loadData(args, split=True):
    ...
    for i, d in enumerate(dev_dir_list):
        ...
        X_data_pd, num_tran, tot_samples, cur_traces= dev_bin_dataset(
        ...
        total_points+=tot_samples
        total_traces += cur_traces
    ...
    print(f"[RYANNNNNNNNNNNNNNNNNNNNNNNNNNNN] Got {total_points} data points from hackerRF day1")
    print(f"[RYANNNNNNNNNNNNNNNNNNNNNNNNNNNN] Got {total_traces} Traces")

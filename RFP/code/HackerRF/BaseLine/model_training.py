...
def main(opts):
    ...
    for idx in idx_list:
        ...
        cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=new_exp + '/best_checkpoint.h5', 
                                                         ...
                                                         mode='auto')
        earlystopping_callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=20, mode='auto', verbose=1)
        ...
        with tf.device('/GPU:0'):
            history = model.fit(train_x, train_y, batch_size=Batch_Size, epochs=150, verbose=1,
                                validation_data=(val_x, val_y), callbacks=[cp_callback, earlystopping_callback])
        train_time = datetime.now() -start 
        print(f"TRAIN TIME: {train_time.total_seconds()} seconds")
        print(f"Epochs: {len(history.history['loss'])} epochs")
        print(f"TIME PER EPOCH: {train_time.total_seconds()/len(history.history['loss'])} sec per epochs")

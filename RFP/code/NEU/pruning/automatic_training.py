...
def main(opts,optsz):
    ...
    for idx in idx_list:
        ...
        print(f"PRUNE TIME: {prune_runtime.total_seconds()}")
        ...
        #finetune_total_epochs = len(history.history['loss'])

        #print(f"Finetune runtime: {finetune_runtime.total_seconds()}")
        #print(f"Finetune total epochs: {finetune_total_epochs}")
        #print(f"Average epoch time: {finetune_runtime.total_seconds()/finetune_total_epochs }")

        best_checkpoint = optsz.new_exp + '/best_checkpoint.h5'
        model_pruned.load_weights(best_checkpoint)

        #prune_runtime = datetime.now() - prune_start
        #print(f"PRUNE TIEM: {prune_runtime.total_seconds()}")
...
if __name__ == "__main__":
    ...
    #TODO: OUR DAY 4 is in a sketchy path... it's saved away
    #           from the rest of the data, porque? - from Ryan
    source = ['our_day4']
    target = ['our_day2']
        ...
        for w in [64]:
            for m in ['resnet']:     
                for p in data:
                    ...
                    #TODO: See comment above about the sketch path - from Ryan
                    dataPath = '/home/mabon/NEU/' + p[0]

                    #TODO: Test path is wrong: Uses HackRF data but we in NEU? - from Ryan
                    #TODO: testPath is actually never used... so it doesn't mater but thats 
                    #       a bug fixing a bug 

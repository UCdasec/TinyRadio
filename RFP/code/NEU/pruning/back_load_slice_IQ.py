...
#    filelist = os.listdir(glob_dat_path)
    ...
    for i,f in enumerate(filelist):
        ...
        if dataType == "IQ":
            IQs_per_tran, n_samples = read_f32_bin(f, start_idx, channel_first)
            ...
            print(type(IQs_per_tran))
            print(f"Len of IQs: {len(IQs_per_tran)}")
            print(f"Shape of IQs: {IQs_per_tran.shape}")
            print(f"Tot samples from read bin : {n_samples}")
            print(f"Running tot : {tot_samples}")
            print(f"Data type: {IQs_per_tran.dtype}")
        ...
        if not mul_trans:
            ...
            break
    #TODO: NOTIVE: Added a return value here 
    return all_IQ_data, num_tran, tot_samples
...
def loadData(args, split=True):
    ...
    for i, d in enumerate(dev_dir_list):
        ...
        X_data_pd, num_tran,cur_samples= dev_bin_dataset(
            os.path.join(p, file_key), 
            n_slices_per_dev, 
            slice_len, 
            start_idx, 
            channel_first, 
        ...
        total_rows += X_data_pd.shape[0]
        total_samples += cur_samples

        total_transmissions += num_tran
        
        x_train_pd, y_train_pd, x_val_pd, y_val_pd, x_test_pd, y_test_pd = utility.splitData(split_ratio, X_data_pd, y_data_pd)

        if i == 0:
            x_train, x_val, x_test = x_train_pd, x_val_pd, x_test_pd
            y_train, y_val, y_test = y_train_pd, y_val_pd, y_test_pd
        else:
    ...
    print(f"[HERRRRRRRRRRRRRRRRRRRRE] Got total rows to be: {total_rows}")
    print(f"[HERRRRRRRRRRRRRRRRRRRRE] Got tranmissions to be: {total_transmissions}")
    print(f"[HERRRRRRRRRRRRRRRRRRRRE] Got samples to be: {total_samples}")
    print(f"[HERRRRRRRRRRRRRRRRRRRRE] Got traces to be: {total_samples/854}")

    print(f"Total rows: {total_rows}")

    return x_train, y_train, x_val, y_val, x_test, y_test, n_devices
